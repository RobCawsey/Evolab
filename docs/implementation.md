...
| 16 | [Reopen and replay](#slice-16--reopen-and-replay) | 1 | **complete** |
...
> **Status: built.** One session. Closes slice 12's trajectory gap without uploading anything —
> and the two bugs it turned up on the way are worth more than the feature.

### Goal

A saved or shared run comes back with its scrubber, its 3D replay and its three gait-analysis
panels. Verified: `scrub` and `gait` both go from off to on when a run is opened, and a
`?shared=<token>` link lands in Explorer with all three.

**Nothing is uploaded.** The trial is deterministic in the genome, the body, the seed and the
length; a run record stores all four, so the recording is recomputed in about fifteen
milliseconds. `POST /api/trajectories` and its content-addressed store stay built, tested and
called by nothing — confirmed at the end of the session: zero mentions in the server log, zero
blobs on disk.

### The seed bug, and the rule it breaks

`spawnPool` never set `trialSeed`, so it fell through to `DEFAULT_CONFIG`'s 0 and every genome
was **scored** at seed 0 — while `respawn()` recomputed the replay at `state.seed`. On the
reference champion that is 5.9394 m and duty 0.7992 scored against 5.9751 and 0.8028 replayed.

**The robot on screen was a different trial from the numbers printed beside it.** Exactly what
slice 10 fixed when an eight-second recording of a four-second trial made the footfall diagram
disagree with the behaviour map cell next to it, and the same rule settles it. One line, and
afterwards the replay's peak distance reads 4.97 m against the Champion panel's 4.97 m.

It had to go first: the whole slice rests on comparing a recomputation against stored numbers,
and this would have made every run report a mismatch for a reason unrelated to what the check is
for.

### Restored as a champion, not as slider contents

That is the whole mechanism. `openSavedRun` used to end with `setMode('manual')`, and manual mode
replays live with nothing to scrub. Restored as a champion, `respawn()` recomputes the recording
exactly as it does for one just evolved, and every panel that needs frames gets them.

`state.champion` gained `result: TrialResult | null` as a field and `summary` became optional,
because a restored champion never came from a generation. **Synthesising a summary was rejected**:
it would have meant inventing a mean, a diversity and an evaluation count that the panels might
then display.

### Three decimals is not enough to reproduce a trial

The first end-to-end test failed, on a run saved minutes earlier in the same session:

```
                stored      recomputed
distance        4.9722      5.0101
strideLength    1.1000      0.8039      <- 27% out
effort          53.7        53.7
```

Not the physics. `encodeGait` rounds to **three decimals**, which is right for a URL and lossy for
a recomputation — the saved gait was not the champion's gait, it was a rounded copy of it.
Recomputing from `1.999` when the value was `1.99937…` lands on a different trial.

Stride moved furthest because it comes from footfall timing: a thousandth of a radian of phase is
enough to merge two touchdowns into one. The same shape as slice 13's bin-boundary bug — rounding
for the wire costs reproducibility — and the same answer: `encodeGaitPrecise` at six decimals for
storage, `encodeGait` unchanged for the URL.

> **Where a number is rounded decides what it can still be used for.** A gait rounded for
> legibility cannot be re-simulated; a behaviour rounded for the wire cannot be re-binned.

### A tolerance tighter than the storage can never be met

With that fixed, one field still reported: `effort was 62.500 and is now 62.534`. `serialise.ts`
rounds effort to **one** decimal while everything else gets four, so a flat thousandth was
unsatisfiable for it and would have reported every run as diverged for ever.

The tolerance is now per-field, at half the last stored digit — 0.05 for effort, 1e-3 for the
rest. It is derived from how `serialise.ts` writes each field, which is the only defensible place
to get it from.

### The note does not name a cause

It says *"This run does not replay exactly — strideLength was 1.100 and is now 0.804. You are
watching a re-simulation from the stored gait, not the run itself."*

An earlier draft blamed the physics. That is one of two possible causes and, as the session
proved, usually the wrong one: the record can simply be too lossy to reproduce from. Runs saved
before this slice still carry three-decimal gaits and still report — correctly, and without being
told a false reason.

Silence is the answer for a match. A run that replays correctly should not announce it.

### Measured

```
seed fix        replay peak 4.97 m against panel 4.97 m   (was 5.9751 vs 5.9394)
reopen          scrub off→on, gait off→on, mode evolved, no divergence note
old run         reports strideLength 1.100 → 0.804, honestly
shared link     Explorer, scrubber, 3D, gait strip, no note
trajectories    0 calls, 0 blobs
tests           330 Node, 30 .NET, golden 6.4598
```

### Done when

- [x] Opening a saved run gives a scrubber, a 3D view and the footfall / traces / portrait strip.
- [x] `?shared=<token>` does the same.
- [x] The replay is the trial that produced the numbers beside it.
- [x] A run that does not reproduce says so — verified against a genuinely lossy older run rather
      than a fabricated one, which is better evidence than the spec asked for.
- [x] `compareTrial` is pure and tested in Node.
- [x] Nothing calls the trajectory endpoint.
- [x] Golden 6.4598 unchanged. In-app fitnesses move, because trials now run at the seed the
      replay uses; that is the point of the fix.

### Deliberately not in this slice

**No trajectory upload.** Still the correct end state. If runs ever need to be an archival record
rather than a reproducible recipe, the endpoint is waiting — and this slice's comparison is what
would tell you the day had arrived.

**No re-rounding of the older runs.** They stay as they are and report honestly. Rewriting stored
data to make a warning go away is the wrong instinct.

**No caching of the recomputed recording.** Fifteen milliseconds does not need one, and a cache
would need invalidating every time the body is edited.
